...
import { Plus, Edit2, CheckCircle, X, Search, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
...
export function AdminView() {
  ...
  const [reservedSoldStep, setReservedSoldStep] = useState('choice');
  const [reservedSoldMode, setReservedSoldMode] = useState(null);
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-2xl border border-gray-200 pointer-events-auto max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-900">
                {reservedSoldStep === 'choice' ? (reservedSoldPlate.isReserved ? 'Mark Available or Sold' : 'Mark Reserved or Sold') : reservedSoldMode === 'reserved' ? 'Mark as Reserved' : 'Mark as Sold'}
              </h2>
            ...
            {reservedSoldStep === 'choice' && (
              <div className="flex gap-4">
                {!reservedSoldPlate.isReserved && (
                  <button type="button" onClick={() => chooseReservedSoldMode('reserved')} className="flex-1 py-6 px-4 rounded-xl font-semibold text-lg bg-amber-100 text-amber-800 border-2 border-amber-400 shadow-md hover:bg-amber-500 hover:text-white hover:border-amber-600 hover:shadow-xl hover:scale-105 active:scale-[0.98] cursor-pointer transition-all duration-200">
                    Reserved
                  </button>
                )}
                {reservedSoldPlate.isReserved && (
                  <button type="button" onClick={() => chooseReservedSoldMode('available')} className="flex-1 py-6 px-4 rounded-xl font-semibold text-lg bg-green-100 text-green-800 border-2 border-green-400 shadow-md hover:bg-green-500 hover:text-white hover:border-green-600 hover:shadow-xl hover:scale-105 active:scale-[0.98] cursor-pointer transition-all duration-200">
                    Available
                  </button>
                )}
                <button type="button" onClick={() => chooseReservedSoldMode('sold')} className="flex-1 py-6 px-4 rounded-xl font-semibold text-lg bg-slate-100 text-slate-800 border-2 border-slate-400 shadow-md hover:bg-slate-600 hover:text-white hover:border-slate-700 hover:shadow-xl hover:scale-105 active:scale-[0.98] cursor-pointer transition-all duration-200">
                  Sold
                </button>
              </div>
            )}
              ...
              <form onSubmit={handleReserveSoldSubmit} className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const today = new Date().toISOString().slice(0, 10);
                      setReserveSoldForm((prev) => ({ ...prev, useCurrentDate: checked, date: checked ? today : '' }));
                    }}
                ...
                {reservedSoldPlate.isReserved && reservedSoldMode === 'sold' ? (
                  <div>
                    ...
                    <input type="text" readOnly value={reservedSoldPlate.contactNumber ? `${reservedSoldPlate.soldReservedBy || '—'} - ${last4Phone(reservedSoldPlate.contactNumber)}` : (reservedSoldPlate.soldReservedBy || '—')} className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-700 cursor-not-allowed" />
                  </div>
                ) : (
                <>
                <div className="relative">
                  <label className="block text-gray-700 mb-2">{reservedSoldMode === 'reserved' ? 'Reserved By' : 'Sold By'} <span className="text-red-500">*</span></label>
                  {reserveSoldDealersLoading ? (
                    ...
                    <div className="flex items-center gap-2">
                      <input type="text" readOnly value={reserveSoldForm.byWho === 'others' ? 'Others' : (() => { const d = reserveSoldDealers.find((x) => String(x.id) === String(reserveSoldForm.byWho)); return d ? dealerDisplayLabel(d) : ''; })()} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-700" />
                      <button type="button" onClick={() => setReserveSoldForm((prev) => ({ ...prev, byWho: '' }))} className="px-3 py-2 border border-gray-300 rounded-lg text-gray-600 hover:bg-gray-50">Change</button>
                    </div>
                  ) : (
                    <>
                      <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
                        <input type="text" value={dealerSearchKeyword} onChange={(e) => { setDealerSearchKeyword(e.target.value); setDealerDropdownOpen(true); }} onFocus={() => setDealerDropdownOpen(true)} onBlur={() => setTimeout(() => setDealerDropdownOpen(false), 200)} placeholder="Search by name, phone, or email..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </div>
                      {dealerDropdownOpen && (
                        <div className="absolute z-10 mt-1 w-full left-0 right-0 bg-white border border-gray-300 rounded-lg shadow-lg overflow-hidden" style={{ height: '192px' }}>
                          <ul className="dealer-dropdown-list py-1 w-full list-none m-0 border-0 block" style={{ height: '192px', minHeight: '192px' }}>
                            {(() => {
                              ...
                              const filtered = kw ? active.filter((d) => [d.fullName, d.phoneNumber, d.email].some((f) => (f || '').toLowerCase().includes(kw))) : active;
                              return (
                                <>
                                  {filtered.length === 0 ? <li className="px-4 py-2 text-gray-500 text-sm">No dealers match</li> : filtered.map((d) => (
                                    <li key={d.id} className="px-4 py-2 cursor-pointer hover:bg-blue-50 text-gray-900" onMouseDown={() => { setReserveSoldForm((prev) => ({ ...prev, byWho: String(d.id) })); setDealerSearchKeyword(''); setDealerDropdownOpen(false); }}>{dealerDisplayLabel(d)}</li>
                                  ))}
                                  <li className="px-4 py-2 cursor-pointer hover:bg-blue-50 text-gray-900" onMouseDown={() => { setReserveSoldForm((prev) => ({ ...prev, byWho: 'others' })); setDealerSearchKeyword(''); setDealerDropdownOpen(false); }}>Others</li>
                                </>
                              );
                            })()}
                          </ul>
                ...
                {reserveSoldForm.byWho === 'others' && (
                  <>
                    <div>
                      ...
                      <input type="text" value={reserveSoldForm.othersFullName} onChange={(e) => setReserveSoldForm((prev) => ({ ...prev, othersFullName: e.target.value }))} placeholder="e.g., John Doe" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-gray-700 mb-2">Contact No <span className="text-red-500">*</span></label>
                      <input type="text" value={reserveSoldForm.othersContact} onChange={(e) => setReserveSoldForm((prev) => ({ ...prev, othersContact: e.target.value }))} placeholder="e.g., 012-3456789" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-gray-700 mb-2">Email <span className="text-gray-400 text-sm">(optional)</span></label>
                      <input type="email" value={reserveSoldForm.othersEmail} onChange={(e) => setReserveSoldForm((prev) => ({ ...prev, othersEmail: e.target.value }))} placeholder="e.g., john@example.com" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  </>
                )}
                </>
                ...
                <div className="flex gap-3 pt-4">
                  <button type="button" onClick={() => { setReservedSoldStep('choice'); setReservedSoldMode(null); }} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Back</button>
                  <button type="submit" disabled={reserveSoldSubmitting} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
